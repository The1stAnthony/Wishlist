...
export default function GiftCard({ item, onPurchase, showPurchase = false, onUndo, currentUserId }) {
    ...
    <div className={`gift-card ${isFullyBought ? 'is-purchased' : ''}`}>
      ...
      <div className="gift-card-body">
        ...
        <div className="gift-card-footer">
          {/* Link always shown — viewer may want the item for themselves */}
          {affiliateUrl && (
            <a href={affiliateUrl} target="_blank" rel="noopener noreferrer" className="gift-card-buy-btn">
              🛒 Shop Now
            </a>
          )}

          {isFullyBought ? (
            <div className="gift-card-purchased-status">
              {quantity === 1 && <span>✅ Purchased</span>}
              {canUndo && (
                <button className="gift-card-undo-btn" onClick={() => onUndo(item.id)}>
                  Undo
                </button>
              )}
            </div>
            ...
            showPurchase && onPurchase && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', flexWrap: 'wrap' }}>
                ...
                >
                  Mark as bought{remaining > 1 ? ` (${buyQty})` : ''}
                </button>
              </div>
            )
          )}
